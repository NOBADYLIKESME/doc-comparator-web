  ...
  const startComparison = async (values) => {
    ...
    try {
      ...
      if (values.format === 'DOCX') {
        console.log('前端检测到DOCX格式，准备发送请求');
        ...
        if (response.data && response.data.type === 'docx') {
          ...
          // 重要：使用IP地址（当前页面内嵌的OnlyOffice已经配置了允许私有IP）
          let documentUrl;
          if (window.location.hostname === 'localhost') {
            // 开发环境 - 使用IP地址
            documentUrl = `http://192.168.77.144:3000/api/compare/preview/${fileId}`;
          } else {
            // 生产环境
            documentUrl = `${window.location.origin}/api/compare/preview/${fileId}`;
          }
        ...
        // 其他格式处理逻辑 - 直接在新窗口中打开
        console.log(`前端检测到${values.format}格式，准备发送请求`);
        
        const response = await axios.post('/api/compare', formData, {
          responseType: 'blob'
        });

        console.log(`${values.format}响应状态:`, response.status);
        ...
        if (response.status === 200) {
          ...
          // 根据格式设置文件名
          const filename = `comparison_result.${values.format.toLowerCase()}`;
